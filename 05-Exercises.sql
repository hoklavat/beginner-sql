-- 05-Exercises
-- exercises.tar database should be restored from pgAdmin.
...
-- SELECT facid, name, membercost, monthlymaintenance FROM cd.facilities WHERE membercost < (monthlymaintenance/50);
...
-- SELECT memid, surname, firstname, joindate FROM cd.members WHERE extract(month FROM joindate) > 7;
...
-- SELECT memid, joindate, surname, firstname FROM cd.members ORDER BY memid DESC LIMIT 1;
...
-- SELECT facid, SUM(slots) FROM cd.bookings WHERE extract(month FROM starttime)=8 GROUP BY(facid) ORDER BY SUM(slots) DESC;
...
-- SELECT cd.facilities.facid, name, cd.bookings.starttime FROM cd.facilities INNER JOIN cd.bookings ON cd.facilities.facid = cd.bookings.facid
-- WHERE cd.facilities.name ILIKE '%Tennis%' AND extract(month FROM cd.bookings.starttime) = 9 AND extract(day FROM cd.bookings.starttime) = 21;

-- SELECT bks.starttime AS start, facs.name AS name FROM cd.facilities facs INNER JOIN cd.bookings bks ON facs.facid = bks.facid WHERE facs.facid IN (0,1) 
-- AND bks.starttime >= '2012-09-21' AND bks.starttime < '2012-09-22' ORDER BY bks.starttime;

-- 15) How can you produce a list of the start times for bookings by members named 'David Farrell'?
-- SELECT cd.bookings.memid, cd.members.firstname, cd.members.surname, cd.bookings.starttime 
